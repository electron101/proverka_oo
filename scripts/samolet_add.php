...
if (!empty($_POST)) 
{
	...
	{	
		...
		if (!($stmt = $mysqli->prepare("INSERT INTO samolet (bort_num, model, company, date_vipusk, colvo_mest) VALUES (?, ?, ?, ?, ?)")))
		{
			echo "invalid";
			exit();
		}
		...
		if (!$stmt->bind_param('ssssi', $bort_num, $model, $company, $date_vipusk, $colvo_mest))
		{
			echo "invalid";
			exit();
		}
