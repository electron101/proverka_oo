...
<!-- Контейнер, содержащий форму обратной связи -->
	<div class="panel panel-info">
		...
		<div class="panel-heading panel-title">
			<h1 class="panel-title">Редактировать класс</h1>
		</div>
		...
		<div class="panel-body">
			...
			<div class="hidden" id="success-alert-btn">
				<a class="btn btn-sm btn-info" href="?act=class" role="button">
					<span class="glyphicon glyphicon-menu-left" aria-hidden="true"></span> Назад</a>
			</div>
			
	<?php
		$result = $mysqli->query("SELECT * FROM class WHERE id = '$id'");
		if ($result)   
			$row = $result->fetch_array()
	?>

			<form role="form" id="ClassEditForm">   				

				<div class="form-group has-feedback">
				  <label for="inputText">Город</label>
				  <input type="text" id="class" name="class" value="<?=$row["name"]?>" class="form-control" placeholder="Название" required autofocus>
...
<script src="js/class_edit.js"></script>
